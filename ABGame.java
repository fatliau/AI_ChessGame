...
public class ABGame {
	...
	public static void main(String []args){
		...
		String output=ABGame(inputString, true, depth);
	...
	public static String ABGame(String testBoard, boolean forMax, int estDepth){
	...
	public static EstNode<Board> MaxMin(EstNode<Board> x,int A, int B, int depthLimit ){
		...
		//mid game
		Board[] moves= result.getData().moveGenerator_midgame(1);//Max means color is white
		...
		if(movesLength==0 || estDepth>=estDepthLimit){//no moves are available in this board TODO bigeer and equal 
			result.setPoint(result.getData().estFunction_MidEnd());
			nodeEvaluated++;
			if(movesLength==0){
				System.out.println("No moves are available in this board: "+result);
				//System.out.println("The data in midgame is: "+result.getData()+"  The point is: "+result.getPoint()+"  The child choice is "+result.getChildChoice());
				return result;
			}
			else{
				//System.out.println("The data in midgame is: "+result.getData()+"  The point is: "+result.getPoint()+"  The child choice is "+result.getChildChoice());
	...
	public static EstNode<Board> MinMax(EstNode<Board> x,int A, int B, int depthLimit ){
		...
		Board[] moves= result.getData().moveGenerator_midgame(2);//Max means color is white
		int movesLength=moves.length;
		if(movesLength==0 || estDepth>=estDepthLimit){//no moves are available in this board TODO bigeer and equal 
			result.setPoint(result.getData().estFunction_MidEnd());
			nodeEvaluated++;
