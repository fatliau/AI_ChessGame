...
public class MIniMaxGameBlack {
	...
	public static void main(String []args){
		...
		String output=MiniMaxGame(inputString, false, depth);
	...
	public static String MiniMaxGame(String testBoard, boolean forMax, int estDepth){
			...
			while(!theStack.isEmpty()){
				...
				if(movesLength==0 || element.getDepth()>=estDepth){//no moves are available in this board TODO bigeer and equal 
					...
					for(int i=0; i<movesLength; i++){
						children[i]=new EstNode<Board>(moves[i],element,moves[i].estFunction_MidEnd());
						nodeEvaluated++;
