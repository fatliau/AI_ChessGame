...
public class MiniMaxOpeningBlack {
	...
	public static void main(String []args){
		...
		String output=MiniMaxOpening(inputString, false, depth);
	...
	public static String MiniMaxOpening(String testBoard, boolean forMax, int estDepth){
		...
		while(!theStack.isEmpty()){
			...
			if(movesLength==0 || element.getDepth()>=estDepth){//no moves are available in this board TODO bigeer and equal 
				...
				for(int i=0; i<movesLength; i++){
					children[i]=new EstNode<Board>(moves[i],element,moves[i].estFunction_Opening());
					nodeEvaluated++;
