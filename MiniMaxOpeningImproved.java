...
public static int nodeEvaluated=0;
	...
	public static void main(String []args){
		...
		String output=MiniMaxOpeningImproved(inputString, true, depth);
	...
	public static String MiniMaxOpeningImproved(String testBoard, boolean forMax, int estDepth){
		...
		while(!theStack.isEmpty()){
			...
			//Check opening count is within 18, 9 as white and 9 as black

			//System.out.println("The data in Opening is: "+element.getData()+"  The point is: "+element.getPoint());
			Board[] moves= element.getData().moveGenerator(element.getMoveColor());//1 as generate WHITE moves  //TODO deal with exchange term
			...
			if(movesLength==0 || element.getDepth()>=estDepth){//no moves are available in this board TODO bigeer and equal 
				...
				for(int i=0; i<movesLength; i++){
					children[i]=new EstNode<Board>(moves[i],element,moves[i].estFunction_imporvedOpening());
					nodeEvaluated++;
